...
impl HighlightColorResolver {
    ...
    fn resolve_spec(
        ...
        match cache.entry(highlight.clone()) {
...
fn autosuggest_parse_command(
    ...
    let Some(jc) = ast.top().as_job_list().unwrap().get(0) else { return None };
    let Some(first_statement) = jc
        .job
        .statement
        .contents
        .as_decorated_statement() else { return None };
                ...
                if c == '\\' {
                    ...
                    if escaped_char == '\0' {
                        ...
                        for i in 0..chars {
                            ...
                            let Some(d) = buffstr.as_char_slice()[in_pos].to_digit(base) else { break };
                            res = (res * base) + d;
                            in_pos += 1;
                        }
...
fn range_is_potential_path(
    ...
    if unescape_string_in_place(
        ...
        result = is_potential_path(
            &token,
            at_cursor,
            &vec![working_directory.to_owned()],
            ctx,
...
fn is_potential_cd_path(
    ...
    if string_prefixes_string(L!("./"), path) {
        ...
        let mut pathsv = match cdpath {
            None => vec![L!(".").to_owned()],
            Some(cdpath) => cdpath.as_list().iter().cloned().collect(),
        };
...
impl<'s> Highlighter<'s> {
    ...
    pub fn highlight(&mut self) -> ColorArray {
        ...
        if self.ctx.check_cancel() {
            return std::mem::replace(&mut self.color_array, Default::default());
        }
        ...
        std::mem::replace(&mut self.color_array, Default::default())
    }
    ...
    fn color_as_argument(&mut self, node: &dyn ast::Node, options_allowed: bool /* = true */) {
        ...
        ) > 0
        {
            ...
            self.color_array[arg_subcmd_start..arg_subcmd_end].copy_from_slice(&subcolors);
        }
    }
    ...
    fn visit_children<'a>(&mut self, node: &'a dyn Node) {
    ...
    fn visit_decorated_statement(&mut self, stmt: &DecoratedStatement) {
        ...
        let Some(command_range) = stmt.command.try_source_range() else { return };
    ...
    fn visit_block_statement<'a>(&mut self, block: &BlockStatement) {
        match &*block.header {
...
fn statement_get_expanded_command(
    ...
    (err == ExpandResultCode::ok).then(|| out_cmd)
}
